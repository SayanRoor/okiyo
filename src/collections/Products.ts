...
export const Products: CollectionConfig = {
  ...
  fields: [
      ...
      tabs: [
        ...
        },
        {
          ...
          fields: [
            ...
            },
            {
              ...
              type: "upload",
              relationTo: "media",
              label: "PNG для виртуальной примерки (с прозрачным фоном)",
              admin: {
                description:
                  "Фото только оправы со снятыми линзами на прозрачном фоне (PNG). Идеально: вид строго анфас, ширина 1200–1800px. Это изображение накладывается на лицо пользователя при включении камеры.",
              },
            },
            {
              name: "enableTryOn",
              type: "checkbox",
              defaultValue: false,
              label: "Включить кнопку «Примерить онлайн»",
              admin: {
                description:
                  "Кнопка виртуальной примерки появится на странице товара, только если этот флаг включён И загружено PNG оправы выше. Если хотя бы одно условие не выполнено — вместо кнопки показывается «Перезвоните мне».",
              },
            },
            {
              name: "gallery",
        ...
        },
        {
          ...
          fields: [
            ...
            // Поле inStock вынесено в sidebar (см. ниже) — оно нужно
            // в один клик и видно на любой вкладке.
          ],
        },
        {
          label: "Публикация",
    ...
    // В наличии / SOLD OUT — главный switch для пометки разобранных моделей.
    // Вынесен в sidebar, чтобы менеджер мог переключить статус одним кликом,
    // не переходя на конкретную вкладку.
    {
      name: "inStock",
      type: "checkbox",
      defaultValue: true,
      ...
      admin: {
        position: "sidebar",
        description:
          "Снимите галочку — модель станет SOLD OUT. Карточка не пропадёт из каталога: фото приглушится, появится плашка «Sold out», цена зачёркнется. На странице товара кнопка «Заказать в WhatsApp» сменится на «Сообщить о поступлении».",
      },
    },
  ],
};
