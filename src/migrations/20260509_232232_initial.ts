...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  CREATE TYPE "public"."enum_leads_status" AS ENUM('new', 'in_progress', 'closed');
  ...
  CREATE TABLE "settings" (
  	"id" serial PRIMARY KEY NOT NULL,
  	"site_name" varchar DEFAULT 'okiyo' NOT NULL,
  	"logo_id" integer,
  	"tagline" varchar,
  	"phone" varchar,
  	...
  	"whatsapp" varchar,
  	"telegram" varchar,
  	"hero_title" varchar DEFAULT 'Мебель для вашего дома',
  	"hero_subtitle" varchar DEFAULT 'Каталог тщательно подобранной мебели — от диванов до спален.',
  	"hero_image_id" integer,
  	"hero_cta_label" varchar DEFAULT 'Смотреть каталог',
